/*
 * Notes:
 * This only works if run as root. Otherwise prints "hid_force_open failed with return code 12"
 * See: http://boxster.ghz.cc/projects/nut/browser/trunk/scripts/udev/README
 *	& http://boxster.ghz.cc/projects/nut/browser/trunk/scripts/udev/nut-usbups.rules.in
 *	This needs to be installed (as root) when the SDA is installed.
 */
...
int main(int argc, char *argv[]) {
	...
	//unsigned short vendor_id  = 0xa1e;
	unsigned short vendor_id = 0x1781;
	//unsigned short product_id = 0x0002;
	//unsigned short product_id = 0x0001;
	unsigned short product_id = 0x0a1e;
	char *vendor, *product;
	 ...
	 */
	fprintf(stdout, "--- Processing command line...\n");
	...
	fprintf(stdout, "--- hid_new_HIDInterface()\n");
	...
	fprintf(stdout, "--- hid_write_identification()\n");
	ret = hid_write_identification(stdout, hid);
	if (ret != HID_RET_SUCCESS) {
		fprintf(stdout,"hid_write_identification failed with return code %d\n", ret);
		// return 1;
	}

	fprintf(stdout, "--- hid_dump_tree()\n");
	ret = hid_dump_tree(stdout, hid);
	if (ret != HID_RET_SUCCESS) {
		fprintf(stdout, "hid_dump_tree failed with return code %d\n", ret);
		//return 1;
	}
	...
	*   if (ret != HID_RET_SUCCESS) {
	*     fprintf(stdout, "hid_set_output_report failed with return code %d\n", ret);
	*   }
	*
	...
	*   if (ret != HID_RET_SUCCESS) {
	*     fprintf(stdout, "hid_get_input_report failed with return code %d\n", ret);
	*   }
	*   // now use the RECV_PACKET_LEN bytes starting at *packet.
	*/

	fprintf(stdout, "--- hid_close()\n");
	ret = hid_close(hid);
	if (ret != HID_RET_SUCCESS) {
	...
	fprintf(stdout, "--- hid_delete_HIDInterface()\n");
	hid_delete_HIDInterface(&hid);

	fprintf(stdout, "--- hid_cleanup()\n");
	ret = hid_cleanup();
	if (ret != HID_RET_SUCCESS) {
